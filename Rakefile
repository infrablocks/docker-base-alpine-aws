...
namespace :prerelease do
  ...
  task :publish, %i[pr_number run_number run_attempt] do |_, args|
    ...
    # (latest_tag bumped by the family's prerelease type), plus a
    # collision-free per-run suffix in the family's tag grammar. Docker
    # tags forbid '+', so we reuse the semver '-'/'.'-separated form and
    # no build metadata. No git tag is created, pushed, or committed —
    # unlike version:bump this only ever tags the built image.
    ...
    image.tag(repo: 'infrablocks/alpine-aws', tag: pr_tag,
              force: true)
    image.push(nil, tag: pr_tag) do |chunk|
